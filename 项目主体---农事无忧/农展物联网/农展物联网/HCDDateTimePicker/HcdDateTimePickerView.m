...
- (void)setTimeBroadcastView
{
    ...
    topView.backgroundColor = [UIColor colorWithHexString:@"0xF5F5F5"];
    
    okBtn = [[UIButton alloc]initWithFrame:CGRectMake(WIDTH-60, 0, 60, 40)];
    ...
    [okBtn setTitleColor:ZJGreenColor forState:UIControlStateNormal];
    [okBtn addTarget:self action:@selector(selectedButtons:) forControlEvents:UIControlEventTouchUpInside];
    okBtn.tag = kOKBtnTag;
    [self addSubview:okBtn];
    
    cancleBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 60, 40)];
    ...
    [cancleBtn setTitleColor:ZJGreenColor forState:UIControlStateNormal];
...
{
    if (self.datePickerMode == DatePickerDatetimeMode) {
        secondScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(WIDTH*0.85, 30, WIDTH*0.15, 200.0)];
    } else if (self.datePickerMode == DatePickerTimeMode) {
        secondScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(WIDTH*0.67, 30, WIDTH*0.33, 200.)];
    }
...
//    dateTimeStr = [dateTimeStr substringToIndex:9];
    NSString * MonthStr = nil;
    NSString * Day = nil;
    
    if (self.curMonth < 10) {
        MonthStr = [NSString stringWithFormat:@"0%ld",self.curMonth];
           } else {
               MonthStr = [NSString stringWithFormat:@"%ld",self.curMonth];
    }
    if (self.curDay < 10) {
        Day = [NSString stringWithFormat:@"0%ld",self.curDay];
    } else {
        Day = [NSString stringWithFormat:@"%ld",self.curDay];
    }
    dateTimeStr = [NSString stringWithFormat:@"%ld-%@-%@ %02ld:%02ld:%02ld",(long)self.curYear,MonthStr,Day,(long)self.curHour,(long)self.curMin,(long)self.curSecond];
